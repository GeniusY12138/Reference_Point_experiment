...
class C(BaseConstants):
    ...
    # total rounds is 32 (42 – 10).  In the demo we override with participant.length.
    NUM_ROUNDS = 32
...
class BdmPage(Page):
    ...
    def vars_for_template(player: Player):
        player.start_time = time.time()  # --- DATA WRITE: start_time ---

        idx = player.round_number - 1
        p = player.participant
        payment_today = p.s2savings[idx]
        monthA, monthB = p.s2monthA[idx], p.s2monthB[idx]
        probA, probB = int(round(p.s2probA[idx] * 100)), int(round(p.s2probB[idx] * 100))
        low, high = min(monthA, monthB), max(monthA, monthB)
